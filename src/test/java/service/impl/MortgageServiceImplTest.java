...
class MortgageServiceImplTest {
    ...
    void getAllMortgages_shouldReturnListOfMortgages() {
        // Given
        final Mortgage mortgage = mock(Mortgage.class);
        final List<Mortgage> mockMortgages = List.of(mortgage);

        when(mortgageDAO.getAllMortgages()).thenReturn(mockMortgages);

        // When
        final List<Mortgage> mortgages = mortgageServiceImpl.getAllMortgages();
    ...
    void saveMortgage_givenExistingUser_shouldSaveSuccessfully() {
        ...
        final Mortgage expected = new Mortgage(0, amount, passport, currentAmount, term);
        ...
        // When
        final Mortgage result = mortgageServiceImpl.saveMortgage(passport, term, amount, currentAmount);
    ...
    void getMortgageById_givenExistingId_shouldReturnMortgage() {
        // Given
        final int id = 1;
        final Mortgage expected = mock(Mortgage.class);

        when(mortgageDAO.getMortgageById(id)).thenReturn(Optional.of(expected));

        // When
        final Optional<Mortgage> result = mortgageServiceImpl.getMortgageById(id);
