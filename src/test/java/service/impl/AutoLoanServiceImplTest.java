...
class AutoLoanServiceImplTest {
    ...
    void getAllAutoLoans_shouldReturnListOfLoans() {
        // Given
        final AutoLoan autoLoan = mock(AutoLoan.class);
        final List<AutoLoan> mockAutoLoans = List.of(autoLoan);
        when(autoLoanDAO.getAllAutoLoans()).thenReturn(mockAutoLoans);

        // When
        final List<AutoLoan> autoLoans = autoLoanServiceImpl.getAllAutoLoans();

        // Then
        assertEquals(1, autoLoans.size());
        assertEquals(autoLoan, autoLoans.get(0));
    ...
    void saveAutoLoan_givenExistingUser_shouldSaveSuccessfully() {
        ...
        final AutoLoan expectedLoan = new AutoLoan(0, amount, currentAmount, term, passport);
        ...
        // When
        final AutoLoan result = autoLoanServiceImpl.saveAutoLoan(passport, amount, currentAmount, term);
    ...
    void saveAutoLoan_givenNonExistentUser_shouldThrowException() {
        ...
        // Then
        assertEquals("User with passport number 'NOT_EXISTING' does not exist", exception.getMessage());
        verify(userDAO).isPassportNumberAvailable(passport);
    ...
    void getAutoLoanById_givenExistingId_shouldReturnLoan() {
        ...
        final AutoLoan expectedAutoLoan = new AutoLoan(autoLoanId, new BigDecimal("10000"), new BigDecimal("5000"), 12, "AA123456");

        when(autoLoanDAO.getAutoLoanById(autoLoanId)).thenReturn(Optional.of(expectedAutoLoan));

        // When
        final Optional<AutoLoan> result = autoLoanServiceImpl.getAutoLoanById(autoLoanId);
